...
export default function NavButtons({current, goToPage}: navProps):React.ReactNode {
  ...
  const navMoviesBtn: navButtonsType = (
      ...
      >
        <path
          ...
          fill={current === "movies" ? "#fff" : "#5A698F"}
        />
      </svg>
  );
  ...
  const navBookmarkBtn: navButtonsType = (
    ...
    >
      <path
        ...
        fill={current === "bookmark" ? "#fff" : "#5A698F"}
      />
    </svg>
  );
    
  return (
    <div role="presentation" className="flex flex-col gap-[75px] items-center">
      <AppLogo/>
      <div role="presentation" className="flex flex-col gap-10">
        {home}
        {navMoviesBtn}
        {navTvSeriesBtn}
        {navBookmarkBtn}
